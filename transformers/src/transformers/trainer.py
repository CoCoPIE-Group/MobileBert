        ...
        if tb_writer is None and is_tensorboard_available() and self.is_world_process_zero():
            ...
            if self.args.rew:
                ...
                if self.args.prune_ratio_config != None:
                    self.tb_writer = SummaryWriter(str(self.args.logging_dir)+'/reweighted_l1_'+'lr_retrain_'+str(self.args.learning_rate)+'_lr_decay_'+str(self.args.lr_decay)+'_'+str(self.args.prune_ratio_config))
                    self.log_name = "reweighted_retraining"
                    self.output_dir_saved_model = self.log_name+'_'+str(self.args.logging_dir.split("/")[1])+'_lr_retrain_'+str(self.args.learning_rate)+'_lr_decay_'+str(self.args.lr_decay)+'_'+str(self.args.prune_ratio_config)
                else:
                    self.tb_writer = SummaryWriter(str(self.args.logging_dir)+'/reweighted_l1_'+'lr_retrain_'+str(self.args.learning_rate)+'_lr_decay_'+str(self.args.lr_decay)+'_'+str(self.args.prune_config_file))      
                    self.log_name = "reweighted_retraining"
                    self.output_dir_saved_model = self.log_name+'_'+str(self.args.logging_dir.split("/")[1])+'_lr_retrain_'+str(self.args.learning_rate)+'_lr_decay_'+str(self.args.lr_decay)+'_'+str(self.args.prune_config_file)
            else:
    ...
    def train(self, model_path: Optional[str] = None):
        ...
        if self.args.rew:
            ...
            for name, param in (model.named_parameters()):
                if param.shape[0] == 30522: continue
            ...
            for i in range(len(layers)):
                ...
                if (self.args.sparsity_type == "irregular"):
                    #rew_layers.append(1 / (conv_layer.data + eps))
                    rew_layers.append(torch.abs(1 / (conv_layer.data + eps)))
                elif (self.args.sparsity_type == "column"):
                    rew_layers.append(torch.abs(1 / (torch.norm(conv_layer.data, dim=0) + eps)))
                elif (self.args.sparsity_type == "filter"):
                    # rew_layers.append(1 / (torch.norm(torch.norm(conv_layer.data, dim=1), dim=[1, 2]) + eps))
                    # rew_layers.append(torch.abs(1 / (torch.sqrt(((conv_layer.data) ** 2).sum(dim=[1, 2, 3])) + eps)))
                    rew_layers.append(1 / (torch.norm(torch.norm(torch.norm(conv_layer.data, dim=-1), dim=-1), dim=-1) + eps))

                elif (self.args.sparsity_type == "block_filter"): # -libn
                    shape = conv_layer.shape
                    conv = conv_layer.reshape(shape[0],-1)

#####################################################
                    #%%%%%%%%original code: mixed width with division, should change
                    #conv.shape[1] 312
                    #block_row_width 8
                    #block_row_division 104
                    #after block_row_division int 39
                    #convfrag 39 
#####################################################               
                    ...
                    convfrag = torch.chunk(conv, block_number, dim=1) #split block_number pieces.   dim 1 means column prune
                    print('convfrag',len(convfrag)) #how many blocks
                    ...
                    if block_number!=block_number_noint:
                        ...
                        conv=torch.cat((torch.tensor(conv),torch.tensor(arrayzero).cuda()),0)
                        #convnew=torch.cat((conv.clone().detach().cpu(),arrayzero),0)
                    ...
                    rew_layers.append(1 / (torch.norm(mat.data, dim=1) + eps))
                elif (self.args.sparsity_type == "vector"): # -libn
                    shape = conv_layer.shape
                    conv = conv_layer.reshape(shape[0],-1)
                    #print('weight.shape', conv.shape)  #([30522, 768]) 
                    #print('block_row_width',block_row_width)
                    ...
                    convfrag_2 = torch.chunk(convfrag, block_number_2, dim=1) #split block_number pieces.   dim 1 means column prune
                    #print('convfrag_2',len(convfrag_2)) #how many blocks

                    mat = None
                    for j in range(len(convfrag_2)):
                        if mat is None:
                            mat = convfrag_2[j]
                        else:
                            mat = torch.cat((mat,convfrag_2[j]),0)

                    rew_layers.append(1 / (torch.norm(mat.data, dim=1) + eps))



                elif (self.args.sparsity_type == "whole_block"): # -libn
                    shape = conv_layer.shape
                    conv = conv_layer.reshape(shape[0],-1)
                    ...
                    block_row_width = 256   #column
                    block_col_width = 1  #filter
                    ...
                    if block_row_division!=block_row_division_noint:
                        ...
                        print('arrayzero',arrayzero.shape)
                        convfrag=torch.cat((torch.tensor(convfrag),torch.tensor(arrayzero).cuda()),0)
                    ...
                    rew_layers.append(1 / (block_norms + eps))
                    #print("!!!! block_norms.shape", block_norms.shape)
                    #print("!!!! whole_block: rew_layer.shape:",(1 / (block_norms + eps)).shape)
                else:
        ...
        for epoch in train_iterator:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if self.args.rew:
                    ...
                    if step == 0 and epoch in milestone:
                        ...
                        for j in range(len(layers)):
                            if (self.args.sparsity_type == "irregular"):
                                ...
                                rew_layers[j] = torch.abs((1 / (layers[j].data + eps)))
                            elif (self.args.sparsity_type == "column"):
                                rew_layers[j] = torch.abs((1 / (torch.norm(layers[j].data, dim=0) + eps)))
                            elif (self.args.sparsity_type == "filter"):
                                # rew_layers[j] = (1 / (torch.norm(torch.norm(layers[j].data, dim=1), dim=[1, 2]) + eps))
                                # rew_layers[j] = torch.abs((1 / (torch.sqrt(((layers[j].data) ** 2).sum(dim=[1, 2, 3])) + eps)))
                                rew_layers[j] = (1 / (torch.norm(torch.norm(torch.norm(layers[j].data, dim=-1), dim=-1), dim=-1) + eps))
                            elif (self.args.sparsity_type == "block_filter"): # -libn
                                ...
                                if conv.shape[1]%block_row_division != 0 :
                                    print("the layer size is not divisible by block_row_division",conv.shape[1], block_row_division)
                                block_number = int(conv.shape[1]/block_row_division)
                                convfrag = torch.chunk(conv, block_number, dim=1) #dim 1 means column prune                               
                                ...
                                block_number_2 = int(len(block_row_norms)/block_row_division)   
                                convfrag_2 = torch.chunk(convfrag, block_number_2, dim=1) #split block_number pieces.   dim 1 means column prune
                                #print('convfrag',len(convfrag_2)) #how many blocks
                                ...
                                rew_layers[j] = (1 / (block_norms + eps))
                                # print("block_norms.shape", block_norms.shape)
                                # print("whole_block: rew_layer[%d].shape:" %(j),rew_layers[j].shape)
                            else:
                                raise SyntaxError("Unknown sparsity type")
                    ...
                    for j in range(len(layers)):
                        #print('j',j)
                        #print("layers_names[j]", layers_names[j])
                        rew = rew_layers[j]
                        conv_layer = layers[j]
                        if (self.args.sparsity_type == "irregular"):
                            l1_loss = l1_loss + 1e-5 * torch.sum((torch.abs(rew * conv_layer)))
                        elif (self.args.sparsity_type == "column"):
                            l1_loss = l1_loss + penalty_factors[layers_names[j]] * torch.sum(rew * torch.norm(conv_layer, dim=0))
                        elif (self.args.sparsity_type == "filter"):
                            # l1_loss = l1_loss + 1e-3 * torch.sum(rew * torch.norm(torch.norm(conv_layer, dim=1), dim=[1, 2]))
                            # l1_loss = l1_loss + 1e-3 * torch.sum(rew * torch.sqrt(((conv_layer.data) ** 2).sum(dim=[1, 2, 3])))
                            l1_loss = l1_loss + 1e-3 * torch.sum(rew * torch.norm(torch.norm(torch.norm(conv_layer, dim=-1), dim=-1), dim=-1))
                        # block-filter:
                        elif (self.args.sparsity_type == "block_filter"): # -libn
                            shape = layers[j].shape
                            conv = layers[j].reshape(shape[0], -1)
                            ...
                            if conv.shape[1]%block_row_division != 0 :
                                ...
                                # raise SyntaxError("block_size error")
                            block_number = int(conv.shape[1]/block_row_division)                
                            convfrag = torch.chunk(conv, block_number, dim=1)
                            ...
                            mat = None
                            for p in range(len(convfrag_2)):
                                if mat is None:
                                    mat = convfrag_2[p]
                                else:
                                    mat = torch.cat((mat,convfrag_2[p]),0)
                            #print("layers_names[j]", layers_names[j])
                            ...
                            # print("!!!!!!!!!!!! To debug:")
                            # print("Current layer:", layers_names[j])
                            # print("rew.shape:", rew.shape)
                            # print("block_norms.shape:", block_norms.shape)

                            l1_loss = l1_loss + penalty_factors[layers_names[j]] * torch.sum(rew * block_norms)
                ...
                tr_loss += self.training_step(l1_loss, model, inputs)
    ...
    def training_step(self, l1_loss: float, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> float:
        """
